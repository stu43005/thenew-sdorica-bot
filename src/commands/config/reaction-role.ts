...
export default class ReactionRoleCommand implements Command {
	...
	public async execute(intr: CommandInteraction, data: EventData): Promise<void> {
		...
		switch (intr.options.getSubcommand()) {
			...
			case 'add': {
				...
				if (!emojiId) {
					await InteractionUtils.send(intr, `Error: 無法解析表情符號: ${inlineCode(emoji)}。`);
					return;
				}
	...
	private async list(intr: CommandInteraction, data: EventData, messageId: string | null): Promise<void> {
	...
	private async add(intr: CommandInteraction, data: EventData, channel: NewsChannel | TextChannel, message: Message, emojiId: string, roleId: string): Promise<void> {
	...
	private async setMode(intr: CommandInteraction, data: EventData, messageId: string, type: ReactionRoleType, limit: number = 0): Promise<void> {
	...
	private async remove(intr: CommandInteraction, data: EventData, message: Message, emojiId: string | null, roleId: string | undefined): Promise<void> {
...
async function addReactionRole(data: EventData, message: Message, emoji: string, roleId: string): Promise<boolean> {
...
async function setReactionRoleMode(data: EventData, messageId: string, mode: ReactionRoleType, limit: number = 0): Promise<boolean> {
	if (!data.guild) return false;
	const reactionRoles: ReactionRole[] = data.guild.reactionRoles ?? [];
...
async function removeReactionRole(data: EventData, message: Message, emojiId: string | null, roleId: string | undefined): Promise<boolean> {
	if (!data.guild) return false;
	const reactionRoles: ReactionRole[] = data.guild.reactionRoles ?? [];
	...
	for (const rrEmoji of rrEmojis) {
		...
		if (rrEmojiIndex > -1) {
			...
			Logger.debug(`[removeReactionRole] message: ${message.id}, emoji: ${rrEmoji.emoji}, role: ${rrEmoji.roleId}`);
		}
	}
