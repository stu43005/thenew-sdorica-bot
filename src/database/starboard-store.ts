...
export class StarboardStore {
	...
	public static async fromGuild(guild: Discord.Guild): Promise<StarboardStore> {
	...
	public async saveItem(item: StarboardDataNew): Promise<void> {
		const db = admin.firestore();
	...
	public async getItem(message: Discord.Message): Promise<StarboardDataNew | undefined> {
		const db = admin.firestore();
		const starboardRef = db
			.collection('starboard')
			.doc(this.guild.id)
	...
	public getTemporarilyTimer(message: Discord.Message): Promise<void> {
		return this.adding[message.id] || Promise.resolve();
	}

	public setTemporarilyTimer(message: Discord.Message, timer: Promise<void>): void {
	...
	public async addStarboardMessage(message: Discord.Message, count: number, starboardMessage: Discord.Message): Promise<void> {
